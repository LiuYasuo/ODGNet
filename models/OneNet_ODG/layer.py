...
class SamePadConv(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels, kt, supports_len, kernel_size, dropout, dilation=1,
                 groups=1, gamma=0.9):
        super().__init__()
        ...
        self.sum = 0
        self.eta = 48
        self.epsilon = 0.5  # 1.45

    def store_grad(self):
        # print('storing grad')
        # print(self.conv.weight.grad)
    ...
    def graphcof(self, time_emb):
        if self.cof.shape == time_emb.shape and torch.norm(self.cof - time_emb.detach()) > self.epsilon:
            q = time_emb.flatten()
            # read
            att = q @ self.GW
            att = F.softmax(att / 0.5)
            ...
            if self.sum > self.eta:
                self.cof = self.tau2 * time_emb + (1 - self.tau2) * old_w.view(time_emb.size())
            else:
                self.sum = self.sum + 1
            return self.cof
        else:
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, skip_channels, layers, kt, supports_len, kernel_size, dilation,
                 dropout=0.3, gamma=0.9):
        super().__init__(),
        ...
        for i in range(self.layers):
            self.convs.append(
                SamePadConv(in_channels, out_channels, skip_channels, kt, supports_len, kernel_size, dilation=dilation,
...
class DilatedConvEncoder(nn.Module):
    def __init__(self, residual_channels, dilation_channels, skip_channels, blocks, layers, dropout, kt, supports_len,
                 kernel_size, gamma):
        super().__init__()
